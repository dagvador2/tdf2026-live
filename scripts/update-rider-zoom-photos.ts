/**
 * Uploads the zoomed-face cutouts ("Détouré zoomé") from the Panini folder to R2
 * and sets each rider's photoZoomUrl — used for the live-map medallion.
 *
 * Usage: npx tsx scripts/update-rider-zoom-photos.ts
 */
...
// Load .env.local manually (no dotenv dependency) — same pattern as update-rider-photos.ts
...
const PHOTOS_DIR =
  "/Users/clemdaguetschott/Desktop/TDF 26/Panini/03_PHOTOS/detoures/Détouré zoomé";
...
async function main() {
  ...
  for (const [fileName, slug] of entries) {
    ...
    const key = `riders/zoom/${slug}-${Date.now()}.png`;

    const url = await uploadToR2(key, buffer, "image/png");
    await prisma.rider.update({ where: { id: rider.id }, data: { photoZoomUrl: url } });

    console.log(`✅ ${fileName.padEnd(22)} → ${slug.padEnd(25)} → ${url}`);
    ok++;
  }

  console.log(`\n🎉 ${ok}/${entries.length} photos zoomées mises à jour.`);
  if (failed.length) {
